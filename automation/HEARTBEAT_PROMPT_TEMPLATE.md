...
LOCAL DISCOVERY AND REVIEW QUEUE: The dashboard-independent local listener performs paginated workspace searches, checks changed DMs, captures high-confidence items, and queues ambiguous candidates. Do not repeat broad workspace searches or enumerate every DM in Codex. At the start of this heartbeat run, execute `node <ABSOLUTE_PROJECT_PATH>/scripts/ledger-cli.mjs list-pending-candidates --limit 500`. For each candidate at or below the ceiling, inspect the complete source/thread and sufficient surrounding context under the applicable ledger rules below. If it qualifies, capture it with the normal ledger command and then execute `node <ABSOLUTE_PROJECT_PATH>/scripts/ledger-cli.mjs resolve-monitor-candidate --dedupe-key <DEDUPE_KEY> --status captured`. If it does not qualify, resolve it with `--status ignored`. Never resolve a candidate when its required Slack context could not be reviewed. Database dedupe makes overlap safe.
