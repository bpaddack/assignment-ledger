...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (typeof payload.archived === "boolean") {
      ...
      result = await env.DB.prepare(`UPDATE my_tasks SET archived = ?, archived_at = ?
        WHERE id = ? AND (? = 0 OR completed = 1)`)
        .bind(Number(archived), archived ? new Date().toISOString() : null, id, Number(archived)).run();
    } else {
      const completed = Boolean(payload.completed);
      result = await env.DB.prepare("UPDATE my_tasks SET completed = ?, completed_at = ? WHERE id = ? AND archived = 0")
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Could not update task" }, { status: 500 });
  }
}
