...
export function readWebexMcpConfig() {
  ...
  for (const line of lines) {
    if (/^\s*\[mcp_servers\.webex\]\s*$/.test(line)) { active = true; continue; }
  ...
  if (!values.command || !Array.isArray(values.args) || !values.args[0]) {
    throw new Error("The Webex MCP connector is unavailable. Run: ghost mcp all, then restart Codex.");
  }
...
export async function connectWebexMcp() {
  const config = readWebexMcpConfig();
  ...
  const client = new Client({ name: "assignment-ledger-webex-fast-monitor", version: "0.1.0" }, { capabilities: {} });
